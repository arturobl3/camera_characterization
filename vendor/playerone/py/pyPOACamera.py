...
# Platform-aware library loading. Put the SDK binary for your OS in
# vendor/playerone/lib/ :
#   - macOS:    libPlayerOneCamera.dylib (+ symlinks) from the Mac SDK tarball
#               (also needs libusb: brew install libusb, then patch @rpath with
#                install_name_tool -change ... see camchar/backends/playerone.py)
#   - Windows:  PlayerOneCamera.dll from the Windows SDK zip (lib/x64 folder)
#   - Linux:    libPlayerOneCamera.so* files from the Linux SDK tarball
...
dll = cdll.LoadLibrary(os.path.join(_lib_dir, _lib_name))
